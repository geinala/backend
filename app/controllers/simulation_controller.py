...
class SimulationController:
    ...
    async def validate_dataset(self, simulation_id: str) -> JSONResponse:
        start_time = time.time()
        wide_event: dict[str, object] = {
            "event_type": "validate_dataset_request",
        ...
        try:
            job = enqueue_job(
                'app.workers.simulation_worker.validate_dataset',
                job_type=JobType.HEAVY,
                job_prefix=JOB_PREFIXES_ENUM.SIMULATION_VALIDATE_DATASET,
                simulation_id=simulation_id
            )
            ...
            return ResponseFormatter.success_with_data(
                ...
                message="Dataset validation has been enqueued for processing",
                status_code=200
            )
